...
import { Copy, Trash2, ClipboardPaste, FlipVertical } from 'lucide-react'
import { Alert, AlertDescription } from "@/components/ui/alert"
import { StyledContent } from '@/app/components/styled-content'

interface UpsideDownTextClientProps {
  content: {
    title?: string;
    description?: string;
    content?: string;
...
export function UpsideDownTextClient({ content }: UpsideDownTextClientProps) {
  const [inputText, setInputText] = useState('')
  const [outputText, setOutputText] = useState('')
  const [error, setError] = useState('')

  const convertToUpsideDown = useCallback((text: string) => {
  ...
  const handleConvert = useCallback(() => {
    ...
    setOutputText(convertToUpsideDown(inputText))
  }, [inputText, convertToUpsideDown])
      ...
      <Card className="bg-zinc-50/50 dark:bg-zinc-900/50">
        <CardHeader>
          <CardTitle className="text-3xl font-bold text-indigo-900 dark:text-indigo-100">
            {content.title || "Upside Down Text Generator"}
          </CardTitle>
        </CardHeader>
