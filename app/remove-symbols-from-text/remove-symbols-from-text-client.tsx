...
interface RemoveSymbolsFromTextClientProps {
content: {
title?: string;
description?: string;
content?: string;
...
export function RemoveSymbolsFromTextClient({ content }: RemoveSymbolsFromTextClientProps) {
const [inputText, setInputText] = useState('')
const [outputText, setOutputText] = useState('')
const [error, setError] = useState('')
const [symbolsToRemove, setSymbolsToRemove] = useState('!@#$%^&*()_+=-`')


const handleRemoveSymbols = useCallback(() => {
setError('')
if (!inputText) {
...
const regex = new RegExp(`[${symbolsToRemove.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&')}]`, 'g')
const result = inputText.replace(regex, '')
setOutputText(result)
}, [inputText, symbolsToRemove])
...
{content.title || "Remove Symbols From Text"}
</CardTitle>
</CardHeader>
...
<div>
          ...
          <Input
            ...
            placeholder="Enter symbols to remove"
            className="mt-2"
          />
        </div>

<Button onClick={handleRemoveSymbols} className="w-full h-12 bg-blue-500 hover:bg-blue-600 text-white">
...
<div className="bg-white dark:bg-zinc-800 p-4 rounded-lg">
<h4 className="text-lg font-semibold mb-2">Symbols at Beginning</h4>
<p className="text-sm text-zinc-600 dark:text-zinc-400 mb-2">Before:</p>
<pre className="text-xs bg-gray-100 dark:bg-gray-700 p-2 rounded">
{"!@#$%^&*()_+=-`HelloWorld"}
</pre>
...
<div className="bg-white dark:bg-zinc-800 p-4 rounded-lg">
<h4 className="text-lg font-semibold mb-2">Symbols at End</h4>
<p className="text-sm text-zinc-600 dark:text-zinc-400 mb-2">Before:</p>
<pre className="text-xs bg-gray-100 dark:bg-gray-700 p-2 rounded">
{"HelloWorld!@#$%^&*()_+=-`"}
</pre>
...
<div className="bg-white dark:bg-zinc-800 p-4 rounded-lg">
<h4 className="text-lg font-semibold mb-2">Only Symbols</h4>
<p className="text-sm text-zinc-600 dark:text-zinc-400 mb-2">Before:</p>
<pre className="text-xs bg-gray-100 dark:bg-gray-700 p-2 rounded">
{"!@#$%^&*()_+=-`"}
</pre>
...
<div className="bg-white dark:bg-zinc-800 p-4 rounded-lg">
<h4 className="text-lg font-semibold mb-2">No Symbols</h4>
<p className="text-sm text-zinc-600 dark:text-zinc-400 mb-2">Before:</p>
<pre className="text-xs bg-gray-100 dark:bg-gray-700 p-2 rounded">
{"HelloWorld"}
</pre>
